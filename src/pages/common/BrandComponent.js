...
const BrandComponent = ({ isFluid }) => {
	...
	const { brandList, isPostBrand, isPutBrand, isDeleteBrand } = useSelector((state) => state.brand);
	...
	const postBrand = () => {
		dispatch(PostBrand(formik.values, token));
	...
	const editBrand = () => {
		dispatch(PutBrand(dataform._id, dataform, token));
			...
			<Card stretch={isFluid}>
				<CardHeader borderSize={1}>
					<CardLabel icon='Alarm' iconColor='info'>
						<CardTitle>Total {' '}'{' '}{brandList && brandList.data.results.length}{' '}'{' '} brands</CardTitle>
					</CardLabel>
					<CardActions>
						<Button
							color='info'
