...
import { PutProductAll } from '../../../redux/actions/products';
...
const EditProduct = () => {
	...
	const [arrThumbnailChildren, setArrThumbnailChildren] = React.useState(
		location.state.item.thumbnail_children,
	);
	...
	console.log(affilateshopList, 'data addilate');
	if (affilateshopList) {
	...
	const deleteItemThumbnailChildren = (e) => {
		setArrThumbnailChildren(arrThumbnailChildren.filter((items) => items.thumbnail !== e));
	};
	...
	const editProductFrom = () => {
		...
		dispatch(PutProductAll(location.state.item._id, data, token));
	};

	const { isPutProduct } = useSelector((state) => state.product);

	console.log(isPutProduct,"isPutProduct")
	useEffect(() => {
		if (isPutProduct === 'success') {
			navigate(-1);
		}
	}, [isPutProduct, navigate]);
		...
		<PageWrapper title={demoPages.sales.subMenu.product.text}>
			...
			<Page container='isfuld' className='m-3'>
				...
				<div className='mt-3' style={{ display: 'flex', justifyContent: 'flex-end' }}>
					{isPutProduct === 'fail' && (
						<p style={{ fontSize: 14, color: 'red' }}> Thay doi that bai</p>
					)}
					<Button color='info' className='col-md-3' onClick={() => editProductFrom()}>
						Edit Product
					</Button>
				</div>
			</Page>
			...
			<Modal isOpen={isOpenEditColor} setIsOpen={setIsOpenEditColor} size='xl'>
				...
				<ModalBody className='px-4'>
					<div className='row g-4 col-md-12'>
						<div className='col-md-3'>
							<FormGroup id='color' label='Color'>
								<Input value={colorVitural} onChange={(e) => setColorVitural(e.target.value)} />
							</FormGroup>
